...
#Data accessed on https://ww2.amstat.org/publications/jse/v9n2/4Cdata.txt on 2/6/18

#Diamonds Simple Linear Regression Analysis

#Read in the data set (Data set cleaned in excel as a csv, and then the file type was mutated to .R)
diamonds <- read.csv(text = '
0.3,D,VS2,GIA,1302
0.3,E,VS1,GIA,1510
0.3,G,VVS1,GIA,1510
0.3,G,VS1,GIA,1260
...
#Code provided by Dr Grimshaw
names(diamonds)<-c("Carat","Color","Clarity","Cert","Price")
tail(diamonds)

plot(Price ~ Carat, data = diamonds, main = "Prices of Diamond by Carats")
plot(log(Price)~log(Carat), data = diamonds, main = "Log Prices of Diamond by Carats")

#Analysis
#Response Variable: lnPrice
#Explanatory Variable: lnCarat

#Model = lnPrice = Beta0 + Beta1 * lnCarat + e~N(0,SIGMA2)
    # Could also write: Price = e^beta0 * Carat^Beta1 * e^e~lognormal (e*) <- this is a multiplicative effect.
...
#Interpretation
#Estimated slope in a log transformed model (not good)
# Pvalue < 0.0001
# For a 1% increase in Carat, we estimate an expected increase in Price of 1.54% (good)

# They want a prediction
# We're predicting lnprice(hat) = 9.13 + 1.54 lnCarat
# Predict for one carat diamond = 9.13
# So Price = e^9.13
...
# R^2 = 0.9572, but this is using log singapore dollares, so it's based on the log transform.

#Compute the "real" R^2
# R^2 = 1 - (sum(yi = yi(hat))^2 / (sum(yi - y(bar))^2)
# coded
1 -sum((diamonds$Price - exp(predict(diamonds.out)))^2) / sum((diamonds$Price - mean(diamonds$Price))^2)

# Compute ABS of accuracy
...
ggplot(plot.df, aes(Carat, Price)) + 
  ...
  ylab("Diamond Price (singapore $)") +
  geom_point() +
  geom_line(aes(y=fit), color = "royalblue") + #fit comes from predict function in plot.df
  geom_line(aes(y=lwr), color = "red", linetype = "dashed") +
  geom_line(aes(y=upr), color = "red", linetype = "dashed")

#Model predicts well for small diamonds, but gets more variable for bigger diamonds
#Suggest collecting more data for bigger diamonds to make the bigger diamond price more predictable

#Strengths, very predictive model with strongly correlated log data few outliers.

#Weaknesses: not very predictive for big carat sizes, and also does not account for other other
